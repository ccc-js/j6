...
describe('Vector', function() {
  ...
  describe('operator', function () {
    it('add, sum, sqrt, max, product, min', function () {
			assert.equal(R.add(3,5), 8)
			assert.equal(R.add(3, c1).str(), '8+2i')
			assert.equal(v1.oadd(v2)[2].str(), '7+2i')
      assert.equal(m.osum().str(), '7+2i')
      assert.equal(v1.product(), 6)
      assert.equal(v1.max(), 3)
      assert.equal(v1.min(), 1)
		})
  })
